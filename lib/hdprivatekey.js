...
var defineErrors = require('./util/errors').defineErrors;
  ...
  if (arg) {
    if (_.isString(arg) || bufferUtil.isBuffer(arg)) {
      ...
      if (_.isObject(arg)) {
        ...
        throw new HDPrivateKey.Error.UnrecognizedArgument(
          'Creating a HDPrivateKey requires a string, a buffer, a json, or an object,'+
...
HDPrivateKey.prototype.derive = function(arg, hardened) {
  if (_.isNumber(arg)) {
    ...
    throw new HDPrivateKey.Error.InvalidDerivationArgument(
      'Invalid derivation argument "'+arg+'", expected number and boolean or string'
    );
  }
};
...
HDPrivateKey.prototype._deriveFromString = function(path) {
  ...
  if (!_.contains(HDPrivateKey.RootElementAlias, steps[0])) {
    throw new HDPrivateKey.Error.InvalidPath(
      'Invalid path for derivation "'+path+'", must start with "m"'
    );
  }
  steps = steps.slice(1);
...
HDPrivateKey.getSerializedError = function(data, network) {
  ...
  if (!(_.isString(data) || bufferUtil.isBuffer(data))) {
    return new HDPrivateKey.Error.InvalidArgument('Invalid argument: Expected string or buffer');
  }
  if (!Base58.validCharacters(data)) {
    return new HDPrivateKey.Error.InvalidB58Char('Invalid Base 58 character in: "'+data+'"');
  }
  try {
    data = Base58Check.decode(data);
  } catch (e) {
    return new HDPrivateKey.Error.InvalidB58Checksum('Invalid Base 58 checksum in: "'+data+'"');
  }
  if (data.length !== HDPrivateKey.DataLength) {
    return new HDPrivateKey.Error.InvalidLength('Invalid length for xprivkey format in: "'+data+'"');
  }
...
HDPrivateKey._validateNetwork = function(data, networkArg) {
  ...
  if (!network) {
    return new HDPrivateKey.Error.InvalidNetworkArgument(
      'Network argument must be \'livenet\' or \'testnet\', got "'+networkArg+'"'
    );
  }
  var version = data.slice(0, 4);
  if (bufferUtil.integerFromBuffer(version) !== network.xprivkey) {
    return new HDPrivateKey.Error.InvalidNetwork(
      'Unexpected version for network: got "'+version+'"'
    );
  }
  return null;
};
...
HDPrivateKey.fromSeed = function(hexa, network) {
  ...
  if (!Buffer.isBuffer(hexa)) {
    throw new HDPrivateKey.Error.InvalidEntropyArgument(
      'Invalid argument: entropy must be an hexa string or binary buffer, got ' + typeof hexa
    );
  }
  if (hexa.length < MINIMUM_ENTROPY_BITS * BITS_TO_BYTES) {
    throw new HDPrivateKey.Error.NotEnoughEntropy(
      'Need more than 128 bytes of entropy, got ' + hexa.length + ' in "' + hexa + '"'
    );
  }
  if (hexa.length > MAXIMUM_ENTROPY_BITS * BITS_TO_BYTES) {
    throw new HDPrivateKey.Error.TooMuchEntropy(
      'More than 512 bytes of entropy is non standard, got ' + hexa.length + ' in "' + hexa + '"'
    );
  }
...
HDPrivateKey.prototype._buildFromBuffers = function(arg) {
  ...
  if (!arg.checksum || !arg.checksum.length) {
    ...
    if (arg.checksum.toString() !== Base58Check.checksum(concat).toString()) {
      throw new HDPrivateKey.Error.InvalidB58Checksum('Invalid Base 58 checksum in "' + concat + '"');
    }
  }
...
HDPrivateKey.Error = {};
defineErrors(HDPrivateKey.Error, [
  'InvalidArgument',
  'InvalidB58Char',
  'InvalidB58Checksum',
  'InvalidChildIndex',
